...
public class CarRepo implements ICarRepo
{
	...
	{
		String query = "UPDATE cars SET model='"+c.getCarModel()+"', manufacturer = '"+c.getManufacturer()+"', color = '"+c.getCarColor()+"', manufacturingDate= '"+c.getManufacturingDate()+"', CC= "+c.getCarCC()+", price= "+c.getCarPrice()+" WHERE carID='"+c.getCarID()+"'";
		try
		{
			dbc.openConnection();
